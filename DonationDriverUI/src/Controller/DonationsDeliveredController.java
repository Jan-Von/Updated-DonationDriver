...
public class DonationsDeliveredController {
    ...
    private void loadDeliveredTickets() {
        String user = LoginController.currentUserEmail;
        if (user == null || user.trim().isEmpty()) {
            showMessageInList("Please log in to see your delivered donations.");
            return;
        }

        try {
            Client client = Client.getDefault();
            String responseXml = client.readTickets(user, "DELIVERED");
            Client.Response response = Client.parseResponse(responseXml);

            if (response != null && response.isOk()) {
                List<String> lines = buildTicketLines(response.message);
                if (lines.isEmpty()) {
                    showMessageInList("No delivered donations yet.");
                } else {
                    setListLines(lines);
                }
            } else {
